...
getConnection <- function(){
  connection = dbConnect(MySQL(), user = 'root', password = 'password123', dbname = 'credit_card_analysis2',
...
getCurrentMonthTransaction <- function(connection){
  ...
  query <- sprintf("select t.transaction_id as tid, t.amount as Amount, t.date_of_transaction as 'Date', t.time_of_transaction as 'Time', t.transaction_type as 'Type', l.location_name as 'Store Name' from user_transaction_user_id_1 as t left join locations as l on l.location_id = t.locationid_id_fk where month(t.date_of_transaction) = month(sysdate()) and t.deleted=1 and l.deleted=1 order by t.transaction_id;");
  rs = dbSendQuery(connection, query);
  currentMonthData = dbFetch(rs, n=-1);
  currentMonthData$Date <-format(as.Date(currentMonthData$Date, "%Y-%m-%d"), "%m/%d/%y");
  print(paste("Log: Data returned for current month's transaction: ", nrow(currentMonthData)));
